...
(function(global){
	var sSTRING = "string",				// constants used for compression optimization
		...
		util = {}, // container for utility functions exposted in $LAB.util for plugins
	...
	var isFunc = util.isFunc = function(func) {
		...
		serializeArgs = util.serializeArgs = function(args) {
			var sargs = [], idx;
...
// cdnLAB.js (CDN fallback adapter for LABjs 1.0+)
// v0.1 (c) Kyle Simpson
// Designed by Kyle Simpson and Brant Burnett
// MIT License

(function(global){
	var orig_$LAB = global.$LAB;
	if (!orig_$LAB) return; // only adapt LABjs with cdnLABjs wrapper if LABjs exists
	
	var sSTRING = "string",					// constants used for compression optimization
		sCDNWAITTIME = "CDNWaitTime",
		bTRUE = true,
		bFALSE = false,
		fSETTIMEOUT = global.setTimeout,
		fCLEARTIMEOUT = global.clearTimeout,
		fNOOP = function(){},
		isFunc = orig_$LAB.util.isFunc,
	...
	function engine(queueExec, opts, getEngine) {
		...
		function getReloads() {
			...
			for (i=0; i<altScripts.length; i++) {
				script = altScripts[i];
				test = script.test;
				badLoad = bFALSE;
				
				if (typeof test === sSTRING) {
					props = test.split("."), current = global;
					
					for (j=0; j<props.length; j++) {
						current = current[props[j]];
				...
				if (badLoad) {
					result.push(
						typeof script.alt === sSTRING ?
							{src: script.alt, type: script.type, charset: script.charset} :
							script.alt
					);
				}				
			}
		...
		publicAPI = {
			script:function() {
				...
				var args = orig_$LAB.util.serializeArgs(arguments), i, f;
				
				for (i=0; i<args.length; i++)
					if (typeof args[i] !== sSTRING && args[i].alt && args[i].test)
					{
						altScripts.push(args[i]);
						hasAlternates = bTRUE;
					}
				
				f = function() { orig_engine = getOrigEngine().script.apply(null,args); };
				if (queueExec)
					exec.push(f);
				else					
					f();

				if (hasAlternates && !queueExec)
					load_wait_interval = fSETTIMEOUT(initialLoadComplete, opts[sCDNWAITTIME]);
				return publicAPI;
			},
			wait:function(func) {
				wait_func = isFunc(func) ? func : fNOOP;

				var wfunc = finalLoadComplete, queue = queueExec, new_engine, f;
				if (hasAlternates) {
					wfunc = initialLoadComplete;					
					queue = bTRUE;
				}
				...
				f = function() { orig_engine = getOrigEngine().wait(wfunc); };
				if (queueExec)
					exec.push(f);
				else
					f();
	...
	global.$LAB = {
		setGlobalDefaults:function(gdefs) { // intentionally does not return an "engine" instance -- must call as stand-alone function call on $LAB
			if (gdefs[sCDNWAITTIME])
				global_cdnWait = gdefs[sCDNWAITTIME];
			orig_$LAB.setGlobalDefaults(gdefs);
		},
		setOptions:function(opts){ // set options per chain
			return engine(bFALSE, opts);
		},
		script:function(){ // will load one or more scripts
